...
- `docs/incidentes.md` logs resolved production incidents (root cause + fix + lesson learned) — check it when diagnosing an outage/infra issue, and add new entries there when one is resolved. E.g. the 2026-09-17 HTTP 522 outage: a replacement prod VM (`oravel-prod-v2`) had no `http-server`/`https-server` GCP network tags, so the VPC firewall silently blocked all inbound 80/443 traffic even though nginx/php-fpm were healthy — fixed by enabling "Allow HTTP/HTTPS traffic" on the instance. `PRE_DEPLOY_CHECKLIST.md` now has a "Nova VM de produção" section for this.
