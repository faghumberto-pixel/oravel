...
- **Don't silently enable recurring/hourly PR-watching loops after opening a PR.** It's fine to fix a real merge conflict or CI failure when asked to check on a PR, but don't turn on an automatic hourly check-in (cron/wakeup loop + `subscribe_pr_activity`) on your own judgment — ask first, or only do it when there's a clear, stated reason. A background loop that mostly reports "nothing changed" every hour is noise the user didn't ask for (real incident: 2026-09-09, ran unprompted for ~24h on PR #18 before the user asked what it was and had it stopped).
