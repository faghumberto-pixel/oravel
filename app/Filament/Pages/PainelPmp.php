...
class PainelPmp extends Page
{
    ...
    /**
     * % de O.S. preventivas concluídas sobre o total -- alimenta o
     * App\Filament\Widgets\Charts\GaugeChart no topo do dashboard.
     */
    public function getTaxaConclusao(): float
    {
        $kpis = $this->getKpis();
        ...
        return round(($kpis['concluidas'] / $kpis['total']) * 100, 1);
    }

    /**
     * Meses abreviados fixos em pt-BR -- ver mesmo comentário que existia em
     * PmpEvolutionChart::MESES_ABREV (Carbon::translatedFormat() não segue
     * o locale do app aqui).
     */
    private const MESES_ABREV = [1 => 'Jan', 'Fev', 'Mar', 'Abr', 'Mai', 'Jun', 'Jul', 'Ago', 'Set', 'Out', 'Nov', 'Dez'];

    /**
     * "Realizado" = O.S. preventivas concluídas (finished_at) no mês.
     * "Planejado" = O.S. preventivas com scheduled_at previsto pro mês
     * (inclui as ainda não concluídas). Alimenta o
     * App\Filament\Widgets\Charts\AreaChart genérico (substitui o antigo
     * PmpEvolutionChart, que fazia essa mesma query só que hard-coded num
     * widget de uso único).
     */
    public function getEvolutionAreaData(): array
    {
        $tenantId = Tenancy::current()?->id;
        $months = collect(range(5, 0))->map(fn ($i) => now()->subMonths($i)->startOfMonth());
        $labels = $months->map(fn (Carbon $m) => self::MESES_ABREV[$m->month])->all();

        $realizado = $months->map(fn (Carbon $month) => MaintenanceOrder::where('tenant_id', $tenantId)
            ->where('maintenance_type', MaintenanceOrder::TYPE_PREVENTIVE)
            ->where('internal_status', 'concluido')
            ->whereBetween('finished_at', [$month, $month->copy()->endOfMonth()])
            ->count())->all();

        $planejado = $months->map(fn (Carbon $month) => MaintenanceOrder::where('tenant_id', $tenantId)
            ->where('maintenance_type', MaintenanceOrder::TYPE_PREVENTIVE)
            ->where('status', '!=', 'Cancelada')
        ...
        return [
            'labels' => $labels,
            'seriesA' => ['name' => 'Realizado', 'color' => '#3987e5', 'data' => $realizado],
            'seriesB' => ['name' => 'Planejado', 'color' => '#c98500', 'data' => $planejado],
        ];
    }

    /**
     * O.S. preventivas marcadas com prazo fatal, por mês de criação --
     * métrica nova (tendência de urgência), não mostrada em nenhum outro
     * gráfico do PMP. Alimenta o LineChartWithMarkers (série única, sem
     * legenda).
     */
    public function getPrazoFatalPorMesData(): array
    {
        $tenantId = Tenancy::current()?->id;
        $months = collect(range(5, 0))->map(fn ($i) => now()->subMonths($i)->startOfMonth());
        $labels = $months->map(fn (Carbon $m) => self::MESES_ABREV[$m->month])->all();

        $dados = $months->map(fn (Carbon $month) => MaintenanceOrder::where('tenant_id', $tenantId)
            ->where('maintenance_type', MaintenanceOrder::TYPE_PREVENTIVE)
        ...
        return [
            'labels' => $labels,
            'series' => [
                ['name' => 'Prazo Fatal', 'color' => '#e6534d', 'data' => $dados],
            ],
        ];
    }
