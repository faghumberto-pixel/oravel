...
class SuppliersCreatedTrendWidget extends LineChartWithMarkers
{
    ...
    public function mount(
        ...
        $dados = $months->map(fn (Carbon $month) => Supplier::whereBetween('created_at', [$month, $month->copy()->endOfMonth()])
            ->count())->all();

        parent::mount(
            labels: $labels,
            series: [
                ['name' => 'Fornecedores', 'color' => '#3987e5', 'data' => $dados],
            ],
            chartTitle: 'Fornecedores Cadastrados por Mês',
        );
    }
}
