...
class StockEntriesVsExitsAreaWidget extends AreaChart
{
    ...
    public function mount(
        ...
        $entradas = $months->map(fn (Carbon $month) => MaterialStockMovement::where('type', MaterialStockMovement::TYPE_ENTRADA)
            ->whereBetween('created_at', [$month, $month->copy()->endOfMonth()])
            ->sum('quantity'))->all();

        $saidas = $months->map(fn (Carbon $month) => MaterialStockMovement::where('type', MaterialStockMovement::TYPE_SAIDA)
            ->whereBetween('created_at', [$month, $month->copy()->endOfMonth()])
            ->sum('quantity'))->all();
