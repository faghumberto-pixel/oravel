...
class MaterialsByCategoryChartWidget extends ChartWidget
{
    ...
    protected function getData(): array
    {
        $counts = Material::query()
            ...
            ->selectRaw('material_categories.name as categoria, count(*) as total')
            ->groupBy('material_categories.name')
            ->orderByDesc('total')
            ->limit(8)
            ->pluck('total', 'categoria');

        $semCategoria = Material::whereNull('material_category_id')->count();
        if ($semCategoria > 0) {
