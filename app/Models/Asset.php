...
class Asset extends Model
{
    ...
    /**
     * Calcula depreciação linear com base em valor de aquisição,
     * valor residual e vida útil em anos.
     */
    ...
    /**
     * Resumo financeiro: custo total de manutenção, receita de locação,
     * valor atual depreciado e resultado (lucro/prejuízo).
     */
    public function getFinancialSummary(): array
    {
        $depreciation = $this->getDepreciationData();

        $totalMaintenanceCost = (float) $this->maintenanceOrders()
            ->sum('total_order_cost');

        $totalLaborCost = (float) $this->maintenanceOrders()
            ->sum('labor_cost');

        $totalMaterialCost = (float) $this->maintenanceOrders()
            ->sum('material_cost');

        $totalLogisticsCost = (float) $this->maintenanceOrders()
            ->sum('logistics_cost');

        $totalRentalRevenue = (float) $this->contracts()
            ->sum('price');
