...
<div class="absolute inset-0 mx-auto flex max-w-md flex-col bg-slate-950">
    ...
    <header class="sticky top-0 z-40 border-b border-slate-800 bg-slate-900/95 backdrop-blur px-5 py-4">
        <div class="space-y-3">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-xs font-semibold text-slate-400 uppercase tracking-wide">Próximos 30 dias</p>
                    <h1 class="text-2xl font-black text-white mt-1">Programação</h1>
                </div>
            ...
            <div class="flex items-center gap-2 text-sm">
                ...
                <span class="font-semibold text-slate-300">{{ count($this->agendaItems) }} agendamentos</span>
            </div>
        </div>
    </header>

    {{-- Busca + Filtros --}}
    <div class="border-b border-slate-800 bg-slate-900/50 px-4 py-3 space-y-3">
        {{-- Campo de busca --}}
        <div class="relative">
            <input wire:model.live="search"
                   type="text"
                   placeholder="Buscar..."
                   class="w-full rounded-lg border border-slate-600 bg-slate-800 px-4 py-2.5 text-sm text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500">
        ...
        <div class="flex gap-2">
            <button wire:click="$set('filterType', '')"
                    :class="$filterType === '' ? 'bg-blue-500 text-white shadow-md' : 'bg-slate-800 text-slate-300'"
            ...
            <button wire:click="$set('filterType', 'appointment')"
                    :class="$filterType === 'appointment' ? 'bg-blue-500 text-white shadow-md' : 'bg-slate-800 text-slate-300'"
                    class="flex-1 rounded-lg px-3 py-2 text-xs font-semibold transition active:scale-95">
                Agendamentos
            </button>
            <button wire:click="$set('filterType', 'order')"
                    :class="$filterType === 'order' ? 'bg-blue-500 text-white shadow-md' : 'bg-slate-800 text-slate-300'"
                    class="flex-1 rounded-lg px-3 py-2 text-xs font-semibold transition active:scale-95">
                O.S.
            </button>
        </div>
        ...
        <div class="flex gap-2">
            <button wire:click="$set('sortBy', 'date')"
                    :class="$sortBy === 'date' ? 'bg-slate-700 text-white' : 'bg-slate-800 text-slate-300'"
                    class="flex-1 rounded-lg px-3 py-2 text-xs font-semibold transition active:scale-95">
                Por Data
            </button>
            <button wire:click="$set('sortBy', 'urgency')"
                    :class="$sortBy === 'urgency' ? 'bg-slate-700 text-white' : 'bg-slate-800 text-slate-300'"
                    class="flex-1 rounded-lg px-3 py-2 text-xs font-semibold transition active:scale-95">
                Por Urgência
            </button>
        </div>
    ...
    <main class="flex-1 overflow-y-auto px-4 py-4 space-y-3">
        @forelse ($this->agendaItems as $item)
            <div @if($item['type'] === 'order')
                ...
                @endif
                class="rounded-2xl bg-slate-800 border border-slate-700 overflow-hidden @if($item['type'] === 'order') cursor-pointer transition active:scale-95 hover:shadow-lg @endif">
                <div class="p-4 space-y-2">
                    ...
                    <div class="flex items-start justify-between">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <p class="text-xs font-bold text-slate-400 uppercase">{{ $item['label'] }}</p>
                                <p class="text-xs text-slate-300">{{ $item['date'] }} às {{ $item['time'] }}</p>
                            </div>
                        </div>
                    ...
                    <div class="space-y-1 text-xs">
                        @if ($item['type'] === 'order' && $item['patrimonio'])
                            <p class="text-slate-400">
                                Patrimônio: <span class="font-semibold text-white"># {{ $item['patrimonio'] }}</span>
                            </p>
                        @endif
                        @if ($item['client'] ?? false)
                            <p class="text-slate-400">
                                Local: <span class="font-semibold text-white">📍 {{ $item['client'] }}</span>
                            </p>
                        @endif
                    </div>
        ...
        @empty
            <div class="flex h-96 items-center justify-center">
                <div class="text-center">
                    ...
                    <p class="text-base font-bold text-white">Sem agendamentos</p>
                    <p class="text-xs text-slate-400 mt-1">Você está em dia nos próximos 30 dias</p>
                </div>
            </div>
        @endforelse
        <div class="h-20"></div>
    ...
    {{-- Botão fixo no fundo --}}
    <footer class="sticky bottom-0 border-t border-slate-800 bg-slate-900/95 backdrop-blur px-4 py-3">
