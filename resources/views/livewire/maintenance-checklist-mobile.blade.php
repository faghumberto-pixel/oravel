<div class="mx-auto flex min-h-screen max-w-md flex-col md:h-full md:min-h-0 md:overflow-y-auto">
    ...
    <header class="flex items-center justify-between px-5 pb-2 pt-6">
        <h1 class="text-xs font-bold tracking-widest text-zinc-400">CHECKLIST DIGITAL</h1>
    ...
    <section class="px-5 pb-4">
        ...
        <div class="mt-3 flex items-center gap-4 text-[11px] font-semibold text-zinc-400">
            <span class="flex items-center gap-1">
                ...
                {{ strtoupper($maintenanceOrder->created_at->translatedFormat('d M Y')) }}
            </span>
            <span class="flex items-center gap-1">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="h-3.5 w-3.5">
                ...
                {{ strtoupper($maintenanceOrder->technician?->name ?? '—') }}
            </span>
            ...
            <div class="overflow-hidden rounded-2xl bg-zinc-900">
                <div class="flex w-full items-center gap-3 px-4 py-4">
                    ...
                    <button type="button" wire:click="toggleComplete('{{ $item->id }}')"
                            class="flex h-11 w-11 shrink-0 items-center justify-center rounded-full transition-colors {{ $item->is_completed ? 'bg-emerald-500 text-zinc-950' : 'bg-zinc-800 text-zinc-600' }}">
