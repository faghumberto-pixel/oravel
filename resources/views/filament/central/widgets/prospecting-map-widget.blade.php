    ...
    <x-filament-widgets::widget>
        <x-filament::section>
            <div class="flex flex-wrap items-center justify-between gap-2 mb-4">
                <h2 class="text-lg font-bold text-gray-900 dark:text-gray-100">Prospecção Planejada</h2>
                <div class="flex items-center gap-4 text-xs text-gray-500 dark:text-gray-400">
                    <span class="flex items-center gap-1.5">
                        <span class="inline-block h-2.5 w-2.5 rounded-full" style="background:#64748b"></span>
            ...
            <div wire:ignore>
                <div
                    x-data="{
                        ...
                        criarMapa() {
                            this.mapa = L.map(this.$refs.mapaProspeccaoPlanejada).setView([-15.7801, -47.9292], 4.5);
                        ...
                        plotarMarcadores(ajustarView) {
                            ...
                            let bounds = [];
                            let iconeLead = this.criarIcone('#64748b');
                            ...
                            this.leads.forEach(lead => {
                                if (lead.latitude && lead.longitude) {
                                    ...
                                    let marcador = L.marker([lat, lng], { icon: iconeLead })
                                        .addTo(this.mapa)
                                        .bindPopup('<b>' + lead.company_name + '</b><br><span style=\'color:#6b7280\'>' + lead.stage_label + '</span><br><a href=\'' + lead.url + '\' style=\'color:#64748b\'>Abrir lead</a>');
                ...
                >
                    <div x-ref="mapaProspeccaoPlanejada" style="height: 500px; width: 100%; border-radius: 8px; z-index: 1;"></div>
                </div>
            </div>

            @if(count($this->getLeads()) === 0)
                <p class="text-xs text-gray-500 mt-3">Nenhum lead em Prospecção com endereço geolocalizado ainda.</p>
            @endif
        </x-filament::section>
    </x-filament-widgets::widget>
</div>
